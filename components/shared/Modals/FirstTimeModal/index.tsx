...
import {
  getUserById,
  updateFirstTimeConsent,
} from '@/lib/actions/user.actions';

export default function FirstTimeModal({ clerkId }: { clerkId: string }) {
  ...
  const getCurrentUserById = async (clerkId: string) => {
    const user = await getUserById(clerkId);
    console.log('user', user);
    if (user && user.isFirstTime) {
      console.log('user?.firstTimeConsent');
      setShowModal(true);
    }
  };

  useEffect(() => {
    getCurrentUserById(clerkId);
    return () => {
      setShowModal(false);
    };
  }, []);
